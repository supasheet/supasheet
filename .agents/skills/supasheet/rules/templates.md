...
- Runs as `security invoker`: the caller needs a `select` grant on the template view AND an `insert` grant + permissive RLS on the target table. Column matching goes through `supasheet.get_columns()`, which reflects the same grants.
...
grant select on app.monthly_payment_template to "x-admin";
...
- Always `security_invoker = true`; `select` grant to the roles that should see it, like any feature view.
